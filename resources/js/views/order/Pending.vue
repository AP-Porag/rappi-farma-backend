...
<!--                                <v-toolbar-title class="text-capitalize">{{$route.meta.title}}'s List</v-toolbar-title>-->
                                <v-toolbar-title class="text-capitalize">All Pending Orders</v-toolbar-title>
                                <v-divider
                                    class="mx-4"
                                    inset
                                    vertical
    ...
    methods: {
        async datatableSearch($e){
            if ($e.length >= 3){
                ...
                await axios.get(`/api/order/search-pending-order/${$e}?page=${this.page}`, {params:{'per_page':this.per_page},headers: { 'Authorization': 'Bearer ' + token }})
                    .then((response)=>{
            ...
            if ($e.length <= 0){
                let token = JSON.parse(window.localStorage.getItem('token'))
                await axios.get(`/api/pending-order?page=${this.page}`, {params:{'per_page':this.per_page},headers: { 'Authorization': 'Bearer ' + token }})
                    .then((response)=>{
                        if (response.data.status != 200){
                            console.log(response.data.status)
                        }else {
        ...
        async paginate($e){
            ...
            let token = JSON.parse(window.localStorage.getItem('token'))
            await axios.get(`/api/order/pending-order?page=${this.page}`, {params:{'per_page':this.per_page},headers: { 'Authorization': 'Bearer ' + token }})
                .then((response)=>{
